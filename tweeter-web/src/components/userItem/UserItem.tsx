import { Link } from "react-router-dom";
import { User } from "tweeter-shared";
import { UseUserNavigationHook } from "../Hooks/UserNavigationHook";
import { UserInfoHook } from "../userInfo/UserInfoHooks";


interface Props {
  user: User;
  featurePath: string;
}

const UserItem = (props: Props) => {
  const { goToUser } = UseUserNavigationHook();
  const { authToken } = UserInfoHook();


  return (
    <div className="col bg-light mx-0 px-0">
      <div className="container px-0">
        <div className="row mx-0 px-0">
          <div className="col-auto p-3">
            <img
              src={props.user.imageUrl}
              className="img-fluid"
              width="80"
              alt="Posting user"
            />
          </div>
          <div className="col">
            <h2>
              <b>
                {props.user.firstName} {props.user.lastName}
              </b>{" "}
              -{" "}
              <Link
                to={`${props.featurePath}/${props.user.alias.startsWith("@") ? props.user.alias.substring(1) : props.user.alias}`}
                onClick={(e) => { e.preventDefault(); goToUser(authToken!, props.user.alias); }}
              >
                {props.user.alias}
              </Link>
            </h2>
          </div>
        </div>
      </div>
    </div>
  );
};

export default UserItem;
